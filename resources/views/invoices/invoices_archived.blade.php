...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ trans('message.invoices') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/  {{ trans('message.Invoice_archive') }}</span>
						</div>
					</div>
								...
								<td>
									<div class="dropdown">
										...
										<div  class="dropdown-menu tx-13">
											...
											<form class="dropdown-item" style="display: inline-block;" action="{{route('restoreInvoice',$invoice->id)}}" method="post">
												@csrf
												@method('PUT')
												<input type="number" name="id" value="{{$invoice->id}}" hidden>
												<button class="dropdown-item" style="padding: 5px;
												
												border-radius: 5px;height:27px;border: none;width:38.97px" onclick = "return confirm('  {{ trans('message.okay_transfer_from') }}')" type="submit"  >
{{ trans('message.Move_from_archive') }}  												</button>
											  </form>	
											...
											<a class="dropdown-item"   href="{{route('printInvoice',$invoice->id)}}" style="padding: 8px 15px;width:100%;height:43px;" > {{ trans('message.Print_an_invoice') }}</a>
										</div>
									</div>
								</td>
							</td>
    ...
    <button style="margin-left:auto;" aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>												</div>
